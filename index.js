var adt = (function () {
  ...
  const nautilusDateParser = {
    ...
    parse(fileName) {
      ...
      const date = /* @__PURE__ */ new Date(`${monthName} ${dayValue} ${year}`);
      if (!isNaN(+date)) {
  ...
  function parseInfoDate(info) {
    ...
    const dateString = dateTaken.includes(",") ? dateTaken : dateTaken + ", " + (/* @__PURE__ */ new Date()).getFullYear();
